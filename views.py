...
expense_sheet_manager = SheetManager(
    gc=gc, spreadsheet=workbook, worksheet=expense_sheet
)
...
def format_msg(data):
    ...
    for item in entries:
        ...
        for discount_entry in item.get("discount", []):
            discount_amount = (discount_entry["cases"] * discount_entry["size"]) + (
                discount_entry["pieces"] * (discount_entry["size"] / 24)
            )
        ...
        for commission_entry in item.get("commission", []):
            commission_amount = (
                commission_entry["cases"] * commission_entry["size"]
            ) + (commission_entry["pieces"] * (commission_entry["size"] / 24))
    ...
    if credits["payable"] > 0:
        summary_rows.append(
            ["SUB", f"Credit Payable: ({credits['payable_desc']})", credits["payable"]]
        )
    if credits["receivable"] > 0:
        summary_rows.append(
            [
                "ADD",
                f"Credit Receivable: ({credits['receivable_desc']})",
                credits["receivable"],
            ]
        )
    ...
    final_total = round(final_amount - total_expenses, 4) + (
        credits.get("receivable") - credits.get("payable")
    )
...
def update_sheet():
    ...
    expense_sheet_info = ValidateExpenseInfo(
        transaction_id=order_id, expenses=data_json["expenses"]
    )
